...
As said before, the philosophy of Sketches is to have apps that do just one task, but do it good. For example, allow configuration, have a nice interface, show the state in the monitor, etc.



Furthermore, it is a good idea to design apps that only measure things or perform actions, and 
* 

Allowing scripting in Sketches would be very difficult. First, a interpreted language should be designed, a parser and interpreter created, everything in a very restricted environment. This should also be easy to be understood and edited in a limited GUI.

Most apps in Sketches f

The approach of Sketches to scripting has three pillars:

1) Apps should be small, and do one task good

2) Apps should be able to expose their state 
